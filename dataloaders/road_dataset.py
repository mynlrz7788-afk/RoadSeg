...
class RoadDataset(Dataset):
    def __init__(self, root_path, dataset_name, mode='train', img_size=1024):
        ...
        if self.mode == 'train':
            self.transform = A.Compose([
                # 🌟 核心修改 1：原图用线性(LINEAR)平滑，掩码用最近邻(NEAREST)保真！
    ...
    def __getitem__(self, idx):
        ...
        # 智能匹配不同数据集的标签名
        file_prefix = img_name.rsplit('.', 1)[0]
        if file_prefix.endswith('_sat'):
            core_name = file_prefix.replace('_sat', '') # 针对 DeepGlobe
            mask_name = core_name + '_mask.png'
        else:
            mask_name = file_prefix + '.tif'            # 针对 马萨诸塞
