...
class AFDANet_Exp3(nn.Module):
    def __init__(self, img_size=1024, num_classes=1):
        super(AFDANet_Exp3, self).__init__()
        ...
        # 🌟 实验3新增：深层动态残差瓶颈层
        self.dynamic_bottleneck = DynamicScaleConv2d(in_channels=filters[3], out_channels=filters[3])
        self.bn_dyn = nn.BatchNorm2d(filters[3])
        self.act_dyn = nn.ReLU(inplace=True)

        # 🌟 实验3新增：带有动态融合的 SGADecoder_Dyn
    ...
    def forward(self, x):
        ...
        # 🌟 对 f4 进行动态残差特征提取
        identity_f4 = f4
        f4_dyn = self.dynamic_bottleneck(f4)
        f4_dyn = self.act_dyn(self.bn_dyn(f4_dyn))
        f4 = identity_f4 + f4_dyn 
