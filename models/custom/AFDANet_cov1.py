...
from models.blocks.ARConv import ARConv  # 引入新模块
...
# (此处为了篇幅简洁，省略 Focus, SADBlock, PCSA, MS_DWConv, MLP, SEAFModule 的代码)
# 请从原来的 AFDANet .py 中将这几个类的代码原封不动地复制到这里
# -------------------------------------------------------------
...
# ================= 核心魔改区：AR_SGADBlock =================
class AR_SGADBlock(nn.Module):
    def __init__(self, in_channels, n_filters):
        super(AR_SGADBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, in_channels // 4, 1)
        self.bn1 = nn.BatchNorm2d(in_channels // 4)
        self.relu1 = nonlinearity

        # 【核心改进1】用两个自适应矩形卷积，替代原有的4个固定方向条形卷积
        # 第一个负责原本的正交组特征（自适应横纵马路）
        self.ar_conv_ortho = ARConv(in_channels // 4, in_channels // 4)
        # 第二个负责原本的对角组特征（利用 ARConv 的仿射变换自动拟合任意倾斜弯道）
        self.ar_conv_diag = ARConv(in_channels // 4, in_channels // 4)

        # 归一化层也相应精简为两个
        self.s1bn = nn.BatchNorm2d(in_channels // 4)
        self.s2bn = nn.BatchNorm2d(in_channels // 4)
    ...
    # ！！！重点在这里：一定要把原本的 def forward(self, x): 改成下面这样 ！！！
    def forward(self, x, epoch=1, hw_range=[1, 18]):
        x_in = self.conv1(x)
        x_in = self.bn1(x_in)
        x_in = self.relu1(x_in)

        # 这里原本可能报错的地方，现在可以正确把 epoch 和 hw_range 传给底层 ARConv 了
        x12 = self.s1bn(self.ar_conv_ortho(x_in, epoch=epoch, hw_range=hw_range))
        x12 = self.eca1(self.relu12(x12)) + x_in

        x34 = self.s2bn(self.ar_conv_diag(x_in, epoch=epoch, hw_range=hw_range))
        x34 = self.eca2(self.relu34(x34)) + x_in

        # 拼接特征
        x_cat = torch.cat((x12, x34), 1)

        x_out = self.conv3(x_cat)
        x_out = self.bn3(x_out)
        x_out = self.relu3(x_out)

        x_out = self.conv4(x_out)
        x_out = self.bn4(x_out)
        x_out = self.relu4(x_out)
        
        return x_out

# ================= 核心魔改区：AR_SGADecoder =================
class AR_SGADecoder(nn.Module):
    def __init__(self, in_channels, filters):
        super(AR_SGADecoder, self).__init__()
        # 【核心改进3】不再需要手工传入 kernel_sizes=[3,5,7]
        # ARConv 会自适应学习马路的最佳感知尺度，这里保留3个并行分支让网络自动学习不同的马路特征集合 (Ensemble效果)
        self.sgad1 = AR_SGADBlock(in_channels, filters // 2)
        self.sgad2 = AR_SGADBlock(in_channels, filters // 2)
        self.sgad3 = AR_SGADBlock(in_channels, filters // 2)
    ...
    def forward(self, x, epoch=1, hw_range=[1, 18]):
        # 传递给底层的 SGADBlock
        x1 = self.sgad1(x, epoch, hw_range)
        x2 = self.sgad2(x, epoch, hw_range)
        x3 = self.sgad3(x, epoch, hw_range)
...
# ================= 实验主网络：AFDANet_cov1 =================
class AFDANet_cov1(nn.Module):
    def __init__(self, img_size=1024, num_classes=1):
        super(AFDANet_cov1, self).__init__()
        ...
        # 换上魔改后的自适应矩形卷积解码器
        self.decoder4 = AR_SGADecoder(filters[3], filters[2])
        self.decoder3 = AR_SGADecoder(filters[2], filters[1])
        self.decoder2 = AR_SGADecoder(filters[1], filters[0])
        self.decoder1 = AR_SGADecoder(filters[0], filters[0])
    ...
    def forward(self, x, epoch=1, hw_range=[1, 18]):
        ...
        # Decoder (传递参数)
        d4 = self.decoder4(f4, epoch, hw_range)
        d4 = d4 + f3
        d3 = self.decoder3(d4, epoch, hw_range)
        d3 = d3 + f2
        d2 = self.decoder2(d3, epoch, hw_range)
        d2 = d2 + f1 
        d1 = self.decoder1(d2, epoch, hw_range)
